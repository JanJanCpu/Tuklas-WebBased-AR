...
Deploying the backend after a change: `cd backend && rm -rf dist && vercel deploy --prod --yes --scope tuklasar`. The `--scope tuklasar` is required — without it the CLI resolves to the personal account context instead of the team, and the deploy fails with `"Not authorized"` even though `vercel whoami`/`vercel teams list` look fine. Env vars (`DATABASE_URL`, `DIRECT_URL`, `JWT_SECRET`, `CLIENT_ORIGIN`) are already set on the Vercel project for production and preview; update via `vercel env add <NAME> <environment> --scope tuklasar`.

GitHub → Vercel auto-deploy is **not** connected for the backend — `vercel git connect --scope tuklasar` fails with "Failed to connect earldulay/Tuklas-WebBased-AR to project" (the Vercel GitHub App isn't authorized for this repo under the `tuklasar` account). Until someone grants that access from the Vercel dashboard (Project → Settings → Git), every backend change needs the manual deploy command above — pushing to `main` alone does not redeploy it.
